...
@LOSS.register('anu_npairs_loss')
class ANU_NPairsLoss(nn.Module):
    def __init__(self, cfg, normalize=False):
        super(ANU_NPairsLoss, self).__init__()
    ...
    def forward(self, feats, labels):
        ...
        for i in range(B):
            ...
            anu_neg_sim = torch.matmul(negs, pos) / self.temperature

            logits_qn = torch.cat([pos_sim.unsqueeze(0), neg_sim], dim=0).unsqueeze(0)
            logits_pn = torch.cat([pos_sim.unsqueeze(0), anu_neg_sim], dim=0).unsqueeze(0)
            
            target = torch.zeros(1, dtype=torch.long, device=device)

            losses.append(F.cross_entropy(logits_qn, target)+F.cross_entropy(logits_pn, target))
