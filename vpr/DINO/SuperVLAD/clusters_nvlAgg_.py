...
def get_Cluster_NV(gsv_whole_set, opt, device):
    ...
    initcache = join(opt.dataPath, 'centroids', opt.arch + '_' + cluster_set.dataset + '_' + str(opt.clusters_num) + '_desc_cen.hdf5')
    with h5py.File(initcache, mode='w') as h5: 
        with torch.no_grad():
            ...
            for iteration, (input, indices) in enumerate(data_loader, 1):
                ...
                image_descriptors = model.forward(input).view(input.size(0), encoder_dim, -1).permute(0, 2, 1)
        ...
        print('====> Clustering..')
        niter = 100
        kmeans = faiss.Kmeans(encoder_dim, opt.clusters_num, niter=niter, verbose=False)
        kmeans.train(dbFeat[...])
