...
def compute_recall_performance(
    ...
    """
    #print('checking:', num_references, num_queries, num_references + num_queries, len(descriptors)); print(hello)
    ...
    # add references
    faiss_index.add(descriptors[:num_references].numpy().astype(np.float32))

    # search for queries in the index
    _, predictions = faiss_index.search(descriptors[num_references:].numpy().astype(np.float32), max(k_values))
