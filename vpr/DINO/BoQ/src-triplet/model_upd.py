...
class BoQModel(L.LightningModule):
    ...
    def compute_loss(self, descriptors, labels):
        miner_outputs = self.trip_miner(descriptors, labels)
        ...
        loss_apn = self.trip_loss(descriptors, labels, miner_outputs_apn)       
        loss_pan = self.trip_loss(descriptors, labels, miner_outputs_pan)
