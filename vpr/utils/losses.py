...
#from pytorch_metric_learning.losses.multi_similarity_loss_p2p import MultiSimilarityLoss_P2P
#from pytorch_metric_learning.losses.lifted_structure_loss_p2p import GeneralizedLiftedStructureLoss
#from pytorch_metric_learning.losses.ntxent_loss_p2p import NTXentLoss
...
def get_loss(loss_name):
    ...
    #if loss_name == 'MultiSimilarityLoss': return MultiSimilarityLoss_P2P(alpha=1.0, beta=50, base=0.0, distance=DotProductSimilarity())
    ...
    if loss_name == 'Lifted': return GeneralizedLiftedStructureLoss(neg_margin=0, pos_margin=1, distance=DotProductSimilarity())
    if loss_name == 'FastAPLoss': return losses.FastAPLoss(num_bins=30)
    if loss_name == 'NPair': return losses.NPairsLoss()
    if loss_name == 'NTXentLoss': return losses.NTXentLoss(temperature=0.07) #The MoCo paper uses 0.07, while SimCLR uses 0.5.
    if loss_name == 'NTXentLoss_P2P': return NTXentLoss(temperature=0.07) #The MoCo paper uses 0.07, while SimCLR uses 0.5.
