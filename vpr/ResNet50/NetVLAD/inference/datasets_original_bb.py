...
class PlaceDataset(data.Dataset):
    ...
    def get_positives(self):
        ...
        self.database_utms = np.array([(path.split("@")[1], path.split("@")[2]) for path in self.database]).astype(float)
        self.queries_utms = np.array([(path.split("@")[1], path.split("@")[2]) for path in self.queries]).astype(float)
           
        if self.positives is None:
            knn = NearestNeighbors(n_jobs=-1)
            knn.fit(self.database_utms)
            self.distances, self.positives = knn.radius_neighbors(self.queries_utms, radius=25) #self.posDistThr)  
