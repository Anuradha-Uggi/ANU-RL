...
sys.path.append('/data/anuradha/Anus-PhD/WACV25_Reviews/VPR/WACV25/Multi-Sim/NetVLAD/MSim-Miner/')
...
class ANU_MSim(nn.Module):
    ...
    def forward(self, embeddings, labels, miner_outputs):
        ...
        for q in range(self.embeddings.shape[0]):
            ...
            if rp>1 and cp>0: p_msk = ANU_MSim.Hardest_ANU_Pairs(q_pos_sim_mat, p_msk, 'pos')
            if rn>1 and cn>0: n_msk = ANU_MSim.Hardest_ANU_Pairs(q_neg_sim_mat, n_msk, 'neg')
...
def plot_2d_contour(X, Y, Z, name):
    # 2D Contours
...
def loss_surface(model, top_eigenvectors, images, labels):
    ...
    print("======= Perturbing weights and computing loss =========")
    #with torch.no_grad():
    ...
    Z = loss_surface
    #plot_3d_heatmap(X, Y, Z, 'Anu_Hrd_r2f.png')
    plot_2d_contour(X, Y, Z, 'ANU_Hrd_nv_contour.png')
...
def main():
    ...
    parser.add_argument('--anu_all_model_path', type=str, default='/data/anuradha/Anus-PhD/WACV25_Reviews/VPR/WACV25/Multi-Sim/NetVLAD/MSim-Miner/ANU-Hardest-Pairs/resnet50/lightning_logs/version_0/checkpoints/resnet50_epoch(39)_step(25040)_R1[0.8933]_R5[0.9435].ckpt')
    ...
    model=Model(opt, checkpoint['state_dict']).to(device)
