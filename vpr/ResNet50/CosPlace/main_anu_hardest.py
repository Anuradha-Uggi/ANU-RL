...
class Upd_MultiSim():
    ...
    def Upd_MSim_Scratch(self):
        ...
        for q in range(self.embeddings.shape[0]):
            ...
            total_loss = total_loss + loss #(loss/pos_loss.shape[0])
            del loss 
...
class VPRModel(pl.LightningModule):
    ...
    def __init__(self,
        ...
        #self.encoder_config = backbone_config
        ...
        # DINOv2
        #self.backbone = helper.get_backbone(backbone_arch, backbone_config, pretrained, layers_to_freeze, layers_to_crop)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = 2 * self.loss_function(descriptors, labels) # Call the loss_function we defined above
...
if __name__ == '__main__':
    pl.utilities.seed.seed_everything(seed=190223, workers=True) #seed-1
    #pl.utilities.seed.seed_everything(seed=190224, workers=True) #seed-2
    #pl.utilities.seed.seed_everything(seed=190225, workers=True) #seed-3
    ...
    datamodule = GSVCitiesDataModule(
        ...
        val_set_names=['pitts30k_test'],#'pitts30k_val', 'pitts30k_test', 'msls_val'], # pitts30k_val, pitts30k_test, msls_val
    )
    ...
    model = VPRModel(
        ...
        #---- Train hyperparameters
        lr=0.025, # 0.0002 for adam, 0.05 or sgd (needs to change according to batch size)
    ...
    trainer = pl.Trainer(
        ...
        default_root_dir=f'./ANU-Hardest-Pairs/TMLR-Rev/{model.encoder_arch}', # Tensorflow can be used to viz 
