...
def feature_match(eval_set, device, opt, config):
    ...
    #write_kapture_output(opt, eval_set, predictions, 'MSNV_cph_predictions_L3.txt')
    ...
    if opt.ground_truth_path is not None:
        ...
        if not opt.msls:
           gt = eval_set.get_positives(opt)
        elif opt.msls:
           print('===> MSLS activated')
           gt = eval_set.pIdx
        global_recalls = compute_recall(gt, predictions, eval_set.numQ, n_values, 'Geotag')
...
def main():
    ...
    #parser.add_argument('--dataset_root_dir', type=str, default='/home/anuradha/A-MyResearch-Sindhu/DataSets/',
    #                    help='If the files in query_file_path and index_file_path are relative, use dataset_root_dir as prefix.')
    parser.add_argument('--dataset_root_dir', type=str, default='/home/anuradha/A-MyResearch-Sindhu/DataSets/VPR-Datasets/Used-Datasets-WACV-25/',
    ...
    config['feature_match']['imageresizeH'] = config['feature_match']['imageresizeW'] = opt.inp_dim
