...
def input_transform(resize=(112, 112)): #(224, 224)
...
class PlaceDataset(data.Dataset):
    ...
    def get_positives(self, opt):
        ...
        if opt.data_name in ('st_lucia', 'eynsham', 'svox', 'svoxvar'):  
            del self.utmQ, self.utmDb
            self.utmDb = np.array([(path.split("@")[1], path.split("@")[2]) for path in self.database]).astype(float)
            self.utmQ = np.array([(path.split("@")[1], path.split("@")[2]) for path in self.queries]).astype(float)
            if opt.data_name == 'st_lucia':
               del self.posDistThr
               self.posDistThr = 5
        ...
        if self.positives is None:
            knn = NearestNeighbors(n_jobs=-1)
            knn.fit(self.utmDb)
            self.distances, self.positives = knn.radius_neighbors(self.utmQ, radius=self.posDistThr) #'5''
        
        if opt.data_name in ('amstertime', 'essex3in1', 'gardens_right', 'gardens_left', 'sped'):    
            del self.positives 
